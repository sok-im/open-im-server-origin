...
# 红包过期通知 API 测试脚本
#
# 路由：POST /msg/notify_red_packet_expired
#
# 依赖：curl / jq
# 用法：
#   SENDER_USER_ID=userA ./scripts/test/notify_red_packet_expired_api_test.sh
...
BIZ_ID="${BIZ_ID:-rp_expired_$(date +%s)}"
TEXT="${TEXT:-你发出的红包已过期，金额已退回}"
DETAIL_URL="${DETAIL_URL:-https://example.com/wallet/redpacket/expired/detail}"
OPERATION_ID="notify-red-packet-expired-api-test-$(date +%s)"
...
echo ">>> 发送红包过期通知"
RESP="$(call_api "/msg/notify_red_packet_expired" "$(cat <<EOF
{
...
echo "$RESP" | jq .
[[ "$(echo "$RESP" | jq -r '.errCode')" == "0" ]] || die "红包过期通知发送失败"
