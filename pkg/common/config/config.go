...
type Notification struct {
	...
	GroupCallParticipantDeclined     NotificationConfig `mapstructure:"groupCallParticipantDeclined"`
	SignalingInvite                  NotificationConfig `mapstructure:"signalingInvite"`
	...
	ConversationE2EE                 NotificationConfig `mapstructure:"conversationE2EE"`
}
...
// AdminAddress 为 chat-admin-api 根地址（如 http://127.0.0.1:10009），用于 POST /account/login 获取 adminToken。
// AdminPassword 留空时默认使用 md5(adminAccount) 的十六进制字符串。
type ChatAPI struct {
	Address       string `mapstructure:"address"`
