...
func (s *userServer) SetDeleteAccountInterval(ctx context.Context, req *pbuser.SetDeleteAccountIntervalReq) (*pbuser.SetDeleteAccountIntervalResp, error) {
	...
	log.ZInfo(ctx, "tommie SetDeleteAccountInterval: success",
		"userID", req.UserID,
...
func (s *userServer) UserRegister(ctx context.Context, req *pbuser.UserRegisterReq) (resp *pbuser.UserRegisterResp, err error) {
	...
	for _, user := range req.Users {
		...
		u := &tablerelation.User{
			...
			AvNotification:             tablerelation.NotificationSwitchOn,
			AvCallRingtone:             tablerelation.NotificationSwitchOn,
			PlayCalleeRingtoneOnAnswer: tablerelation.NotificationSwitchOn,
		}
...
func (s *userServer) AddNotificationAccount(ctx context.Context, req *pbuser.AddNotificationAccountReq) (*pbuser.AddNotificationAccountResp, error) {
	if err := authverify.CheckAdmin(ctx, s.config.Share.IMAdminUserID); err != nil {
		log.ZError(ctx, "AddNotificationAccount: CheckAdmin failed", err,
	...
	if err := s.db.Create(ctx, []*tablerelation.User{user}); err != nil {
		log.ZError(ctx, "AddNotificationAccount: Create failed", err,
			"opUserID", mcontext.GetOpUserID(ctx), "userID", req.UserID)
		return nil, err
	}
...
func (s *userServer) NotificationUserInfoUpdate(ctx context.Context, userID string, oldUser *tablerelation.User) error {
	...
	if user.Nickname == oldUser.Nickname && user.FaceURL == oldUser.FaceURL &&
		...
		log.ZInfo(ctx, "NotificationUserInfoUpdate skip no profile change",
			"userID", userID)
		return nil
	}
	log.ZInfo(ctx, "NotificationUserInfoUpdate profile changed",
		"userID", userID,
