...
func (s *userServer) updateOfflineRecord(ctx context.Context, userID string, hasOffline, hasOnline bool) {
	...
	if len(platformIDs) == 0 {
		...
		log.ZInfo(ctx, "updateOfflineRecord: Upsert", "userID", userID, "offlineTime", offlineTime, "deadline", deadline)
		...
		log.ZInfo(ctx, "updateOfflineRecord: Delete", "userID", userID)
		// 用户重新上线，删除离线记录，停止计时
