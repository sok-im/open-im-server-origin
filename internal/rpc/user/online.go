...
func (s *userServer) updateOfflineRecord(ctx context.Context, userID string, hasOffline, hasOnline bool) {
	...
	if len(platformIDs) == 0 {
		...
		log.ZInfo(ctx, "tommie updateOfflineRecord: Upsert", "userID", userID, "offlineTime", offlineTime, "deadline", deadline)
		if err := s.userOfflineRecord.Upsert(ctx, userID, offlineTime, deadline); err != nil {
			log.ZWarn(ctx, "tommie updateOfflineRecord: Upsert failed", err, "userID", userID)
		}
	} else if hasOnline {
		log.ZInfo(ctx, "tommie updateOfflineRecord: Delete", "userID", userID)
		// 用户重新上线，删除离线记录，停止计时
		if err := s.userOfflineRecord.Delete(ctx, userID); err != nil {
			log.ZWarn(ctx, "tommie updateOfflineRecord: Delete failed", err, "userID", userID)
		}
	}
}
